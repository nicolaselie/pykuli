...
class PyKeyboard(PyKeyboardMeta):
    ...
    def press_key(self, character='', modifier=0):
        ...
        """
        vk_key_scan = win32api.VkKeyScan(character)
        if vk_key_scan == -1:
            return False
            
        char_vk, modifier_vk = [ord(s) for s in struct.pack('<h', vk_key_scan)]
        modifier_vk |= modifier
        if (modifier_vk & FLAG_SHIFT) == FLAG_SHIFT:
            win32api.keybd_event(self.shift_key, 0, 0, 0)
        if (modifier_vk & FLAG_CTRL) == FLAG_CTRL:
            win32api.keybd_event(self.control_key, 0, 0, 0)
        if (modifier_vk & FLAG_ALT) == FLAG_ALT:
            win32api.keybd_event(self.alt_key, 0, 0, 0)
    ...
    def release_key(self, character='', modifier=0):
        ...
        if (modifier_vk & FLAG_CTRL) == FLAG_CTRL:
            win32api.keybd_event(self.control_key, 0, KEYEVENTF_KEYUP, 0)
        if (modifier_vk & FLAG_ALT) == FLAG_ALT:
            win32api.keybd_event(self.alt_key, 0, KEYEVENTF_KEYUP, 0)
    ...
    def special_key_assignment(self):
        ...
        self.alt_gr_key = self.alt_r_key
