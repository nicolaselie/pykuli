...
class KeyboardMeta(object):
    ...
    def tap_key(self, character='', n=1, interval=0, modifier=0):
        """Press and release a given character key n times."""
        for i in xrange(n):
            self.press_key(character, modifier=modifier)
            self.release_key(character, modifier=modifier)
            time.sleep(interval)

    def type_string(self, char_string, interval=0, modifier=0):
        """A convenience method for typing longer strings of characters."""
        for i in char_string:
            time.sleep(interval)
            self.tap_key(i, modifier=modifier)
